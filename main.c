...
float loss_function(Xor model, Matrix training_input, Matrix training_output){
    ...
    for(size_t i = 0; i < input_rows; i++){ //for each row in training input
...
int main(){
    ...
    Matrix X_sample = matrix_initialize(1, 2);
    matrix_choose_rows(X_sample, X, 3, 4);
    matrix_copy(nn.as[0], X_sample);
